    ...
    if(isset($_POST['title']))
    {
        ...
        if($err==0){
            ...
            if(empty($_FILES['image']['tmp_name']))
            {
                ...
                require "../connexion.php";
                $update = $bdd->prepare("UPDATE works SET title=:titre, date=:date, description=:description WHERE id=:myid");
                $update->execute([
                    ":titre" => $title, 
                    ":date" => $date, 
                ...
                if(!isset($erreur))
                {
                    ...
                    if(move_uploaded_file($_FILES['image']['tmp_name'], $dossier.$fichiercptl))
                    {
                        ...
                        require "../connexion.php";
                        $update = $bdd->prepare("UPDATE works SET title=:titre, date=:date, description=:description, image=:image WHERE id=:myid");
                        $update->execute([
                            ":titre" => $title, 
                            ":date" => $date, 
                            ":description" => $description, 
                        ...
                        $update->closeCursor();
                        header("LOCATION:works.php?updatesuccess=".$id);
                    }else{
                        header("LOCATION:updateWorks.php?id=".$id."&errorimg=3");
                    }             
                }else{
                    header("LOCATION:updateWorks.php?id=".$id."&errorimg=".$erreur);
                }
